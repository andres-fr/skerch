...
r"""Parallelized measurements
=================

One core strength of sketched methods, particularly useful when dealing with
slow linops, is the ability to parallelize matrix-vector measurements.

Skerch supports this in several ways:

* **Batched matmul**: Users can invoke measurements on the whole measurement
  matrix ``m`` at once, or in batches of specified size. In this case, skerch
  simply calls ``lop @ m`` with matrices ``m`` and expects the provided ``lop``
  to handle this matrix-matrix operations accordingly.

* **Indexed matmul**: If the ``lop`` doesn't support matrix-matrix
  parallelization, or a subset of measurements is needed (e.g. when
  parallelizing across different machines), users can specify which indexes
  should be ran.

Consider the case when a linear operator supports individual matrix-vector
multiplications, which are not memory-expensive but take a long time to
compute. In that case, it is beneficial to
