...
def dim_rank_decay_sym_diags_meas_defl_rtol(request):
    ...
    * ``diag_idxs`` is a collection of integers specifying the diagonal indices
      to be tested: 0 is the main diagonal, 1 the diagonal above, -1 the
      diagonal below, and so on.
...
def dim_rank_decay_sym_subdiags_meas_defl_rtol(request):
    """Test cases for subdiagonal estimation.

    More lightweight than the diagonal counterpart.
    """
    dims, rank = 1000, 100
    if request.config.getoption("--quick"):
        dims, rank = 500, 50
    result = [
        (dims, rank, 0.5, True, [-1, 1], round(dims * 0.995), 0, 0.02),
        # # fast-decay: just Hutch does poorly, but better if symmetric
        # (dims, rank, 0.5, True, [0], round(dims * 0.995), 0, 0.02),
        # (dims, rank, 0.5, False, [0], round(dims * 0.995), 0, 0.2),
        # # slow-decay: just Hutch behaves the same as with fast decay
