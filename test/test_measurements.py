...
# ##############################################################################
def test_get_measblock_correctness(
    rng_seeds,
    torch_devices,
    dtypes_tols,
):
    ...
    for seed in rng_seeds:
        for dtype, tol in dtypes_tols.items():
            meas_lops = [
                ...
                SsrftNoiseLinOp(meas_hw, seed, norm="ortho", register=False),
            ]
            ...
            for device in torch_devices:
                ...
                for mat, lop in zip(meas_mats, meas_lops):
                    mat2 = get_measblock(range(meas_hw[1]), lop, dtype, device)
                    try:
                        assert torch.allclose(
                            mat, mat2, atol=tol
                        ), "Incorrect get_measblock? {lop}"
                    except:
                        # torch.allclose(mat, mat2, atol=tol)
                        breakpoint()
